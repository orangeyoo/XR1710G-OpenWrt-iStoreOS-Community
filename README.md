...
### 切换 LuCI 语言 / Change the LuCI language

首次启动默认是简体中文。英文用户登录 LuCI 后进入 **系统 → 系统 → 语言和界面**，
将 **语言** 改为 **English**，点击 **保存并应用** 后刷新页面。也可以直接打开
LuCI 的 **System → System → Language and Style**。这个操作只改变管理界面语言，
不会修改 WAN、LAN、Wi-Fi 或 Mesh 配置。英文基础界面由 LuCI 自带；本项目的
自定义页面以英文 msgid 作为英文显示，并提供中文翻译包。
