...
| `xr1710g-community-v1.2.0-sysupgrade.itb` | 唯一的系统固件 | OpenWrt/iStoreOS 后台升级，或 YYH2913 U-Boot 的 **Firmware + UBI 2.0 - 439 MiB** 页面安装 |
| `SHA256SUMS.txt` | 文件完整性校验 | 刷写前核对，不刷入路由器 |
| `FLASHING-GUIDE.md` | 本说明 | 先读，再选择刷机路径 |

### 路径 A：已运行兼容 OpenWrt/iStoreOS

在“备份/升级”页面上传 `xr1710g-community-v1.2.0-sysupgrade.itb`。跨发行版或排查旧配置问题时不要保留配置。
...
4. 上传 `xr1710g-community-v1.2.0-sysupgrade.itb`。
...
刷机前确认型号为 Gemtek XR1710G，并核对 SHA-256。**首次启动没有管理员密码，2.4/5GHz Wi-Fi 也是开放网络。请先用网线单独连接，立即设置 root 密码和无线加密，再接入家庭网络。**首次管理地址为 `192.168.50.1`，用户名 `root`。两台新刷设备不要同时接入同一网络，应逐台设置主路由和节点地址。
...
| `xr1710g-community-v1.2.0-sysupgrade.itb` | The only system image | Use from compatible OpenWrt/iStoreOS, or from YYH2913 U-Boot with **Firmware + UBI 2.0 - 439 MiB** |
...
For YYH2913 HTTP U-Boot, open `http://192.168.255.1/`, select **Firmware**, select **UBI 2.0 - 439 MiB**, and upload `xr1710g-community-v1.2.0-sysupgrade.itb`. Do not power off while erasing, writing, or rebooting.
...
Confirm the device is a Gemtek XR1710G and verify the SHA-256 values. **A clean first boot has no administrator password and the 2.4/5GHz Wi-Fi networks are open. Connect one unit by Ethernet, immediately set the root password and wireless encryption, and only then attach it to the home network.** The initial address is `192.168.50.1` and the user is `root`. Configure two freshly flashed units one at a time to avoid duplicate addresses and DHCP servers.
