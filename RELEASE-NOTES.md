...
- **LuCI 无线配置误回退修复**：应用 5GHz DFS 频宽（如 EHT160，需约 60 秒 CAC）时，LuCI 默认 90 秒确认窗口可能先到并自动回滚配置，表现为"160 改回 80"。现出厂将 `luci.apply.rollback` 提升为 300 秒；已有自定义值不覆盖。该修复不改变 v1.5 已修复的后台 CAC 启动故障本身。
